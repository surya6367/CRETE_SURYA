...
function playSound(audioElement) {
    if (audioElement && (audioElement.src.includes('sounds/'))) {
        ...
        // This is crucial for making sounds work after the user's first click.
...
function displayMessage(element, message, type) {
    ...
    // Clear message faster for click feedback, but keep error/success for 5s
...
const app = {
    showPage: (pageId) => {
        // NOTE: playClickSound() is called manually on interactive elements for better control
    ...
    // 4. NEW FEATURE: Save FF UID (Used in slotPage)
    saveFFUid: async () => {
        playClickSound();
        ...
        if (!uidToSave || uidToSave.length < 5 || isNaN(uidToSave)) { // Basic validation: min 5 chars, must be a number
             displayMessage(messageDiv, 'Please enter a valid Free Fire UID (only numbers).', 'error');
             return;
        }
        
        try {
             await db.ref(`users/${currentUser.uid}/ffUid`).set(uidToSave);
             currentUser.ffUid = uidToSave; // Update global state
             displayMessage(messageDiv, 'Free Fire UID saved successfully!', 'success');
             
             // If currently on the slot page, force a refresh to update button states
    ...
    updateMatchTimer: (matchId, matchData, mode) => {
        ...
        // 1. FOCUSED TIMER: Update slot page timer display
        ...
        if (timeDiffSeconds > 0) {
            ...
            if (isCurrentlyViewing) {
                ...
                // 3. SOUND: 59 Second Countdown Sound (runs from 59 down to 2)
            ...
            if (liveTime > 0) {
                ...
                timeString = `LIVE - ${liveTime}s to Clear`;
            } else {
                 // Next 10 seconds: ENDED, countdown to match deletion
                timeString = `ENDED - Deleting in ${timeToDeleteSeconds}s`;
            }
            ...
            // Room Reveal and Start Sound Logic
            if(isCurrentlyViewing) {
                 if (timeDiffSeconds === 0) {
                     playCriticalSound(); // Sound effect for reveal time
    ...
    backToMatchList: () => {
        playClickSound();
        // Stop listening to the current match details
    ...
    listenForSlots: (mode, matchId) => {
        ...
        matchRef.on('value', (snapshot) => {
            ...
            // 2. ROOM ID/PASS ACCESS CONTROL: Reveal only if joined AND started
            if (userInMatch && isMatchStarted) {
